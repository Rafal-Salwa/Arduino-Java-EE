 ...
 *****************************************************************************/
//constructor for the main Epoch class (POLISH VERSION)
...
{
    this.daylist = new Array('Ni', 'Po', 'Wt', 'Śr', 'Cz', 'Pt', 'So', 'Ni', 'Po', 'Wt', 'Śr', 'Cz', 'Pt', 'So');
    ...
    this.clearbtn_caption = 'Czyść';
    this.clearbtn_title = 'Usuwa wszystkie wybrane daty w kalendarzu';
    this.maxrange_caption = 'Osiągnięto koniec obsługiwanego zakresu';
};
//-----------------------------------------------------------------------------
    ...
    if (!owner.selCurMonthOnly || cell.date.getMonth() == owner.displayMonth && cell.date.getFullYear() == owner.displayYear)
    {
        ...
        else //if we can only select one cell at a time
        {
            ...
            if (owner.tgt) //if there is a target element to place the value in, do so
            {
                ...
                form = document.getElementById('id_datdok');
                form2 = document.getElementById('id_datdwe');
                form3 = document.getElementById('id_datksp');
                form4 = document.getElementById('id_datksv');
                if (owner.tgt.value != form.value) {

                    if (form2.value != form3.value) {
                        area = document.getElementById('id_uwagi');
                        area.value = 'Uwzględniono do obliczenia podatku: ' + form3.value;
                    }

                } else {
                    form2.value = form.value;
                    form3.value = form.value;
                    form4.value = form.value;
                }
